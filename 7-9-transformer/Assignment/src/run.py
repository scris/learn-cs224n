...
if __name__ == '__main__':
    # For multi-processing compatibility
    freeze_support()
    ...
    if args.function == 'pretrain':
        ...
        wiki_file = open('wiki.txt', encoding='utf-8').read()
        corruption_dataset = dataset.CharCorruptionDataset(wiki_file, 128)
        tconf = trainer.TrainerConfig(max_epochs=650, batch_size=128,
                                      learning_rate=args.pretrain_lr, lr_decay=True,
                                      warmup_tokens=512*50, final_tokens=200*len(pretrain_dataset)*block_size,
                                      num_workers=4, writer=writer,
                                      ckpt_path=args.writing_params_path)
        trainer = trainer.Trainer(gpt, corruption_dataset, None, tconf)
        trainer.train()
        ...
        train_text = open(args.finetune_corpus_path, 'r', encoding='utf-8').read()
        train_dataset = dataset.NameDataset(pretrain_dataset, train_text)
        if args.reading_params_path is None:
            tconf = trainer.TrainerConfig(max_epochs=75, batch_size=256,
                                          learning_rate=args.finetune_lr, lr_decay=True,
                                          warmup_tokens=512*50, final_tokens=200*len(pretrain_dataset)*block_size,
                                          num_workers=2, writer=writer,
                                          ckpt_path=args.writing_params_path)
